...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        # Compute the objective (log action_vector and entropy)    maximize: sum(td_loss*logP(at|st)) + entropy_weight * entropy  
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]]) #(None, 7, 91)
            ...
            '''
            split = tflearn.conv_1d(inputs, 128, 1, activation = 'relu')   #(None, 7, 128)
            ...
            dense_net_2 = tflearn.fully_connected(split_flat, 256, activation='relu')
            raw1 = tflearn.fully_connected(dense_net_2, K_DIM, activation='softmax')
            raw2 = tflearn.fully_connected(dense_net_2, BETA_DIM, activation='softmax')
            
            out = tf.concat([raw1,raw2],1)
